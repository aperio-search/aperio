...
> [!WARNING]
Treat this page as a **narrative companion** for developers who enjoy reading about low level engineering, not as operational documentation you would rely on for debugging or performance tuning. **If something here contradicts the code, the code wins.<br><br>This document was generated by a large language model** that analyzed the code for this project but has only undergone a superficial human review. Technical details, crate names, code paths, and design rationale may be inaccurate, hallucinated, or out of date with the actual source in `src/`. **The source code is the single source of truth.**
