...
async function runConstantQpsBenchmark(): Promise<void> {
  ...
  while (Date.now() - startTime < DURATION_MS) {
    ...
    // Safety Valve: If the server bogs down and hits max concurrency,
    // back off briefly before firing the next scheduled request.
    ...
    // Pacing logic: Calculate how long the current loop tick took
    // and sleep for the remaining time required to maintain the QPS target.
  ...
  // Drain outstanding queries before closing up shop
